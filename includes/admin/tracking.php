<?php
/**
 * Tracking functions for reporting plugin usage to the KBS site for users that have opted in
 *
 * @package     KBS
 * @subpackage  Admin
...
class KBS_Tracking {
	...
	/**
	 * The data to send to the KBS site
	 *
	 * @access private
	 */
	private $data;
	...
	private function tracking_allowed() {
		return (bool) kbs_get_option( 'allow_tracking', false );
	} // tracking_allowed
	...
	private function setup_data() {
		...
		$data['kbs_version']  = KBS_VERSION;
		$data['wp_version']   = get_bloginfo( 'version' );
		$data['server']       = isset( $_SERVER['SERVER_SOFTWARE'] ) ? $_SERVER['SERVER_SOFTWARE'] : '';

		$submission_page      = kbs_get_option( 'submission_page', false );
        $install_date         = get_option( 'kbs_installed' );
		$data['install_date'] = $install_date ? $install_date : 'not set';
	...
	/**
	 * Send the data to the KBS server
	 *
	 * @access private
	 * @return void
	 */
	public function send_checkin( $override = false, $ignore_last_checkin = false ) {
		...
		if ( $home_url === 'https://kb-support.com/' || apply_filters( 'kbs_disable_tracking_checkin', false ) ) {
			return false;
		}
		...
		$request = wp_remote_post( 'https://kb-support.com/?kbs_action=checkin', array(
			'method'      => 'POST',
			'timeout'     => 20,
			'redirection' => 5,
			'httpversion' => '1.1',
			'blocking'    => true,
			'body'        => $this->data,
			'user-agent'  => 'KBS/' . KBS_VERSION . '; ' . get_bloginfo( 'url' )
		) );
		...
		update_option( 'kbs_tracking_last_send', time() );
	...
	private function get_last_send() {
		return get_option( 'kbs_tracking_last_send' );
	} // get_last_send
	...
	public function schedule_send() {
		...
		add_action( 'kbs_weekly_scheduled_events', array( $this, 'send_checkin' ) );
	} // schedule_send
	...
	public function admin_notice() {
		$hide_notice = get_option( 'kbs_tracking_notice' );

		if ( $hide_notice ) {
			return;
		}

		if ( kbs_get_option( 'allow_tracking', false ) ) {
			return;
		}
		...
		/*if (
			...
			$optin_url  = add_query_arg( 'kbs-action', 'opt_into_tracking' );
			...
			echo '<div class="updated"><p>';
				...
				echo '&nbsp;<a href="' . esc_url( $optout_url ) . '" class="button-secondary">' . __( 'Do not allow', 'kb-support' ) . '</a>';
			echo '</p></div>';
		//}
	}

} // KBS_Tracking
$kbs_tracking = new KBS_Tracking;
